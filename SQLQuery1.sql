...
Create view CountryData as
	with CountryWise as(
		...
		where continent is not null
		group by location, population
	) Select *, ROUND((cast(Cases as float) / Population) * 100, 2) as InfectionRate, 
	ROUND((cast(Deaths as float) / Cases) * 100,2) as MortalityRate
	from CountryWise;

Go

Select * from CountryData order by InfectionRate desc;
------------------------------------------------------------------------------------------------------------------------------
